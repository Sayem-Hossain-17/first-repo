...
void detectcircles(struct Node* p, int count, int current, int start, int* mark, int markCount) ;
int isVisited(int *mark, int nid, int Count);
...
int main() {
    ...
    for (int i = 0; i < nodecount; i++) {
        int mark[50] = {0}; // Initialize visited array
        mark[0] = i; // Start the search from node i
        detectcircles(nodes, nodecount, i, i, mark, 1);
    }
    return 0;
}
int isVisited(int* visited, int nid, int count) {
...
//write circle found emplemantion like below  
void detectcircles(struct Node* p, int count, int current, int start, int* mark, int markCount) {
    if (markCount >= 3 && current == start) {
        printf("Circle found: ");
    ...
    for (int i = 0; i < p[current].adjcount; i++) {
        int adjNode = p[current].adjs[i];
        if (isVisited(mark, adjNode, markCount)!=0) {
            mark[markCount] = adjNode;
            detectcircles(p, count, adjNode, start, mark, markCount + 1);
        }
    }
}
